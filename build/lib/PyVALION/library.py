...
def download_GIRO_parameters(time_start,
    ...
    Returns
    -------
    output_flag : array-like
        Array of bool format, has True if data was present, False if not.
    ...
    for iname in range(0, ion_name.size):
        ...
        output_file_txt = os.path.join(PyVALION.giro_data_dir,
                                       file_name_str + '.txt')
        output_file_pic = os.path.join(PyVALION.giro_data_dir,
                                       file_name_str + '.p')
...
def filter_GIRO_parameters(time_start,
    ...
    ion_name : array-like
        String arrays of GIRO ionosondes names
    filter_CS : flt
        Minimum accepted Autoscaling Confidence Score (from 0 to 100, 999 if manual scaling, -1 if unknown)
    ...
    for iion in range(0, ion_name.size):
        ...
        imput_file_pic = os.path.join(PyVALION.giro_data_dir, file_name_str)
        ...
        if data_new3['fof2'].size > 10:
            for key in data_all:
                data_all[key] = np.concatenate((data_all[key], data_new3[key]), axis=None)

    # Convert datetime to datetime in array form using pandas
    data_all['dtime'] = pd.to_datetime(data_all['dtime'])

    # Pickle file
    output_file_str = 'Filtered_independent_ionosondes.p'
    write_file = os.path.join(PyVALION.giro_data_dir, output_file_str)
    pickle.dump(data_all, open(write_file, "wb"))
...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
def find_G_and_y(time_start, time_finish, in_model_resolution,
                 alon, alat,
                 data,
                 pickle_outputs=True):
    ...
    time_finish : datetime.datetime
        End date and time for the validation period
    in_model_resolution : int
        Model resolution in minutes.
    alon : array-like
    ...
    """
    # Array of time in the validation period (can be several days)
    dtime = datetime.timedelta(minutes=in_model_resolution)
    time_array = pd.to_datetime(np.arange(time_start, time_finish, dtime))

    # Days in the validation period
    a_dates = np.unique(time_array.date)
    ndays = a_dates.size

    # Cycle through each day in the validation period
    for day in range(0, ndays):
        print('Working with day: ', a_dates[day])

        # Array of time frames and boundaries for the data selection
        adtime = np.arange(a_dates[day],
                           a_dates[day] + datetime.timedelta(days=1),
                           datetime.timedelta(minutes=in_model_resolution))

        adtime = pd.to_datetime(adtime)
        adtime0 = adtime - datetime.timedelta(minutes=in_model_resolution) / 2.
        adtime1 = adtime + datetime.timedelta(minutes=in_model_resolution) / 2.
        day_point = adtime[0].strftime('%Y%m%dT0000Z')
        ...
        for it in range(0, adtime.size):
            ...
            for iion in range(0, un_ion.size):
                ...
                if data['dtime'][a[b]].size > 0:
                    ...
                    ion_lon = PyIRI.main_library.adjust_longitude(np.unique(data['lon'][a[b]])[0], 'to180')
        ...
        ay_nmf2 = PyVALION.library.freq2den(ay_fof2) / 1e6
        ...
        if pickle_outputs == True:
            # Pickle y file
            file_name_str = ('y_' +
                            time_start.strftime('%Y%m%dT%H%MZ') +
                            '_' +
                            time_finish.strftime('%Y%m%dT%H%MZ'))
            output_file_pic_y = os.path.join(PyVALION.giro_data_dir,
                                        file_name_str + '.p')
            pickle.dump((ay_nmf2, ay_hmf2,
                        ay_B0, ay_B1,
                        ay_lon, ay_lat,
                        ay_name, ay_time),
                        open(output_file_pic_y, "wb"))
            ...
            file_name_str = ('G_' +
                            ...
                            '_' +
                            time_finish.strftime('%Y%m%dT%H%MZ'))
            output_file_pic_G = os.path.join(PyVALION.giro_data_dir,
                                        file_name_str + '.p')
            pickle.dump(G, open(output_file_pic_G, "wb"))

    return (ay_nmf2, ay_hmf2, ay_B0, ay_B1, ay_lon,
            ay_lat, ay_name, ay_time, G)